...
namespace PhanMemQuanLyDoanThu
{
    class NhanVien
    {
        ...
        {
            string strSQL = string.Format("SELECT TENNV,TENBOPHAN,TENCHUCVU,LUONGCOBAN,LUONGCOBAN*0.2 AS PHUCAP,LUONGCOBAN+(LUONGCOBAN*0.2) AS TONG FROM NHANVIEN A,BOPHAN B,CHUCVU C, LUONG D WHERE A.MANV=D.MANV AND A.MABOPHAN = B.MABOPHAN AND A.MACHUCVU=C.MACHUCVU");
            DataTable dt = db.Execute(strSQL);
            return dt;
        }
    }
